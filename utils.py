...
def download_glad_granule(
    filename: str, layer: str = "lossyear", version: str = "GFC-2024-v1.12"
) -> bool | None:
...
def calculate_window_dates(sos_date: str, eos_date: str) -> tuple[str, str, str, str]:
    """
...
def download_crop_calendars(crop_calendar_dir: str = "data") -> bool:
    ...
    for files in crop_calendar_files.values():
...
def get_best_images(
    ...
    def find_best_image(
        start_date: str, end_date: str, cloud_threshold: int, s2_tile_id: str
    ) -> Any | None:
...
def show_previews(a: Any, b: Any) -> None:
...
def get_tile_id(ft: dict[str, Any]) -> str | None:
    return ft.get("properties", {}).get("Name")


def pick_mgrs_tile(tile_id: str | None) -> None:
    ...
    def highlight_tile(fid: str | None) -> None:
    ...
    def select_tile(fid: str | None) -> None:
    ...
    # ----------------------------
    def on_grid_click(**kwargs: Any) -> None:
...
def get_selected_tile_id() -> str | None:
...
def download_and_run(tile_id: str, season: str, year: int) -> dict[str, str]:
    ...
    image_filename = os.path.join(base_dir, f"ftw_input_{tile_id}_{season}_{year}.tif")
    output_filename = os.path.join(base_dir, f"ftw_predictions_{tile_id}_{season}_{year}.tif")
    filtered_output_filename = os.path.join(
        base_dir, f"ftw_predictions_filtered_{tile_id}_{season}_{year}.tif"
    )
    boundaries_filename = os.path.join(base_dir, f"ftw_boundaries_{tile_id}_{season}_{year}.gpkg")
    ...
    start_tif = os.path.join(base_dir, crop_calendar_files[season]["start"])
    end_tif = os.path.join(base_dir, crop_calendar_files[season]["end"])
    ...
    if filter_result.returncode != 0:
        print("filter-by-lulc failed; continuing with unfiltered predictions.")
        if filter_result.stdout:
            print(filter_result.stdout)
        if filter_result.stderr:
            print(filter_result.stderr)
        filtered_output_filename = output_filename
